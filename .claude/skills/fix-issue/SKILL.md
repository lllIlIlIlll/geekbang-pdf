...
description: >
  ...
  "失败", "崩溃", "线上告警", or similar problem-related keywords.
...
项目提供诊断和验证脚本，位于 `scripts/` 目录：

```bash
# 诊断 Cookie 问题
python3 -c "from scripts.helper import diagnose_issue; print(diagnose_issue('pdf_blank'))"

# 诊断认证问题
python3 -c "from scripts.helper import diagnose_issue; print(diagnose_issue('login_failed'))"

# 诊断选择器问题
python3 -c "from scripts.helper import diagnose_issue; print(diagnose_issue('content_truncate'))"

# 验证 Cookie 修复
python3 -c "from scripts.validator import validate_fix; print(validate_fix('cookie'))"

# 验证 PDF 修复
python3 -c "from scripts.validator import validate_fix; print(validate_fix('pdf', 'out/test.pdf'))"

# 验证选择器修复
python3 -c "from scripts.validator import validate_fix; print(validate_fix('selector', 'article_title'))"
```
...
完整反模式 → **[resources/anti-patterns.md](resources/anti-patterns.md)**

---

## 典型使用示例

### Example 1: PDF Generation Failure

**User says:** "PDF 生成出来是空白的"

**Skill follows these steps:**
1. Request error message and reproduction steps
2. Diagnose cookie validity via `_check_cookie()`
3. Check if page structure changed via `_check_selectors()`
4. Establish acceptance criteria and wait for confirmation
5. Make minimal fix if needed
6. Verify with `_validate_pdf()` script
7. Update CLAUDE.md with lessons learned

### Example 2: Login Redirect Loop

**User says:** "登录成功后一直循环重定向"

**Skill follows these steps:**
1. Request error message and reproduction steps
2. Diagnose authentication state via `_check_auth()`
3. Check selectors.json for redirect-related selectors
4. Establish acceptance criteria and wait for confirmation
5. Make minimal fix if needed
6. Verify fix with test run
7. Update CLAUDE.md with lessons learned

### Example 3: Batch Download Interrupted

**User says:** "批量下载到一半就中断了"

**Skill follows these steps:**
1. Request error message and last successful URL
2. Check urls_batch.txt format and accessibility
3. Verify network connectivity
4. Establish acceptance criteria and wait for confirmation
5. Fix issue (may be network, file format, or code bug)
6. Verify with test batch run
7. Update CLAUDE.md with lessons learned
